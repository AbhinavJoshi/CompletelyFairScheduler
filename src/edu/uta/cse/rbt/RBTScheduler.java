...
public class RBTScheduler {

	public static void main(String[] args) {
		
		InputData data = new InputData("C:/Users/Sandeep/Desktop/TestFiles/newTestFiles/1000.txt");
		//InputData data = new InputData("C:/Users/Sandeep/Desktop/TestFiles/input1.txt");
		...
		RedBlackTree<Task> rbt = new RedBlackTree<Task>();
		ArrayList<TimeData> runResults = schedule(rbt, data, inputTime);
		String taskList = "<";
		for (TimeData td : runResults) {
			...
			if (td.runningTask != null) {
				...
				System.out.println("Number of Tasks in Execution: "
						+ td.numberOfTasks);
				System.out.println("running Task: " + td.runningTask.taskName);
			...
			if (td.runningTask != null)
				System.out.println("Pre order: " + td.treeState);
	...
	public static ArrayList<TimeData> schedule(RedBlackTree<Task> rbt,
		...
		for (int curTime = 0; curTime < inputTime; curTime++) {
			...
			while (taskIndex < data.tasksCount
				...
				rbt.insert(new_task);
				tresults.numberOfTasks = rbt.size()
			...
			// min_vruntime.
			if (running_task == null && rbt.size() > 0) {
				RedBlackNode<Task> min_node = rbt.extractMin();
				running_task = min_node.key;
				rbt.remove(min_node);
				if (rbt.size() > 0) {
					min_vruntime = rbt.extractMin().key.runTime;
				}
			}
			...
			tresults.treeState = rbt.printTree();
